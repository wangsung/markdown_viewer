...
- **워크트리 전환 시 최신화 및 미커밋/타 브랜치 동기화 점검 지침 (Worktree Switch & Sync Policy)**:
  - 작업 영역(Worktree)을 전환하거나 새로 진입할 때는 대상 워크트리의 최신 이력 및 작업 상태(`git status`, `git log`)를 먼저 점검합니다.
  - **동기화 반영 사항 자동 점검 및 병합 제안 (Unmerged Feature Sync Check & Merge Suggestion)**:
    - 전환 대상 워크트리에 `main` 브랜치 또는 타 서브 워크트리(`feature/*`)에서 새롭게 구현/수정된 커밋 중 미반영된 동기화 대상이 있는지 확인(`git log <current_branch>..<other_branch>`)합니다.
    - 미반영 커밋이나 동기화 대상 변경사항이 존재하는 경우, 사용자에게 반영할 커밋 목록 및 동기화 필요성을 명확히 안내하고 **`git merge` 동기화를 우선 제안**해야 합니다.
  - **미커밋 작업물이 없는 경우**: `main` 브랜치 등 상위 브랜치 및 동기화 대상 브랜치와의 최신 동기화(`git merge main` 또는 `git merge <feature_branch>`)를 진행합니다.
  - **미커밋 작업물이 존재하는 경우**:
...
- **메인 브랜치 개발 제한 및 워크트리전용 개발 지침 (Main Branch Restriction & Worktree-Only Policy)**:
  - **메인 브랜치(`main`) 작업 범위 제한**: `main` 브랜치/메인 워크트리에서는 서브 워크트리(`feature/*`)의 작업 결과물을 검증하고 병합(merge) 및 `commit` / `push` 하는 관리 작업 외에는 **직접적인 신규 기능 개발이나 코드 직접 수정을 수행하지 않습니다.**
  - **워크트리 전용 개발 원칙**: 신규 기능 개발, 버그 수정, 코드 리팩토링 등 제반 소스코드 변경 작업은 반드시 적절한 **서브 워크트리(Worktree)**를 통해서만 진행해야 합니다.
  - **워크트리 추천 및 사전 승인 절차**: 사용자로부터 개발/수정 요청을 받았을 때 `main` 브랜치에 있는 경우, 요청 과제에 적합한 **기존 서브 워크트리 추천** 또는 **신규 서브 워크트리 생성 제안**을 사용자에게 명확히 안내하고 확인/승인을 받은 뒤 해당 워크트리 영역에서 작업을 진행해야 합니다.
